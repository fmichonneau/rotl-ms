...
resources OTL provides could be used in education and outreach (e.g.,
...
or use topological trees to identify phylogenetically equivalent species to
increase overlap between chronograms and species trait data
\cite{Pennell2015}. Without branch lengths, these trees remain useful to
illustrate relationships among species, or to map a trait on a phylogeny.

\texttt{rotl} can also be used to download trees accompanying studies that have
been submitted through the curator interface, and identify the trees that are
being used to assemble the synthetic tree. These trees provide a useful resource
to reproduce or expand on an already published analysis.
...
\url{https://cran.r-project.org/package=rncl}. Using NCL provides efficient
parsing of large trees that may contain singletons nodes labeled with taxonomic
information.
...
Requesting a topology from a list of taxa takes place in two steps: the taxa
need to be matched to their identifiers in the Open Tree Taxonomy before
requesting the relationships among them. The first step is performed using the
TNRS.
...
<<>>=
tt <- studies_find_studies(property = "ot:focalCladeOTTTaxonName", "mammalia")
@
