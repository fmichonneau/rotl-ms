...
\noindent{\normalsize \sc Fran\c{c}ois Michonneau$^1$, Joseph Brown$^2$, David Winter$^3$}\\
\noindent {\small \it $^1$Florida Museum of Natural History \& Whitney
  Laboratory for Marine Sciences, University of Florida, Gainesville,
  FL, USA}\\
\noindent{\small \it $^2$University of Michigan, Ann Arbor, MI, USA} \\
\noindent{\small \it $^3$Arizona State University, Tempe, AZ, USA} \\
\end{center}
\medskip
...
(\url{http://tree.opentreeoflife.org}). Additionally, the project provides
...
\citealt{Paradis2004}, phytools \citealt{Revell2012}, geiger
...
comprehensive list). Furthermore, \texttt{rotl} joins a growing list of packages
that aim at querying and accessing data from the web directly in R for analysis.
...
branch lengths information to these topological trees
...
<<plot_taxa, fig.height=4, cache=TRUE, fig.cap="The phylogenetic tree returned by OTL for the list of model species used as an example.">>=
...
<<plot_cats, fig.height=4.2, echo=FALSE, fig.cap='Phylogeny of the Felidae published in \\citealt{Johnson2006} and retrieved from OTL using \\texttt{rotl}'>>=
...
accessible using APIs, several R packages have been developed to take advantage of
...
Figure~\ref{fig:cat_map}, we show how we can obtain a map of the occurrences for
...
<<cat_map, fig.height=4.2, echo=FALSE, results='hide', message=FALSE, cache=TRUE, fig.cap='GBIF records for the species in \\textit{Lynx} included in the phylogeny associated with the study by \\citealt{Johnson2006}'>>=
...
## Make a data frame of the results, and clean up the data with missing locality data
