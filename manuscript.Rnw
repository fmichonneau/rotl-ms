...
community phylogenetics analyses \citep[e.g.,][]{Pearse2015},
\citep{Kembel2010}, but they are often incomplete or not resolved enough. As OTL
becomes more comprehensive, and its taxonomic resolution increases, it could
become a valuable resource for ecologists seeking to use phylogenetic
...
Currently there are seven studies focused on this family available from OTL, and a
single tree is associated with each one. We can then retrieve the study and tree
identifiers associated with the first of these studies, and pass them to the 
function \texttt{get\_study\_tree} to have the tree in memory:
...
\texttt{traits}][]{Chamberlain2015}, \texttt{rotl} provides a way to easily
