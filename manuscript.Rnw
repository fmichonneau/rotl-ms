...
species (genus \textit{Lynx}) that were included in the phylogeny retrieved from
the Felidae study above. We extract the species names from the phylogeny, and use
them to ask for the records for these species found in GBIF
(Figure~\ref{fig:cat_map}). We include the code to reproduce this figure in the
Appendix.
