...
single tree is associated with it. We can then retrieve the identifiers and pass
them to the function \texttt{get\_study\_tree} to have the tree in memory:
...
\texttt{list\_trees} returns a list to retrieve the tree identifiers for each
study easily. Alternatively, the function \texttt{get\_study} returns all the
trees associated with a particular study. Additional details about the study can
be obtained using the function \texttt{get\_study\_meta}.
