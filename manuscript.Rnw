...
These packages, combined the language's support for literatre programming 
\hl{(Knuth\_1984, Xie\_2014)} make R an ideal platform for reproducable research
in phylogentics and comparative biology, as they allow a complete record of the 
...
phylogentic and taxonomic data from Open Tree of Life in R. \texttt{rotl} takes
adavantage of Application Programming Interfaces (APIs) provided by OTL to acess
...
users to acess phylogenies stored in treebase. \texttt{rotl} contributes to this
initiative, and greatly extends the number of taxa for which phylogenetic data
can be retreived within R while allowing the  data from OTL can be combined with 
...
can be gathered and visualized. Another vigenette titled ``meta-analysis'' 
