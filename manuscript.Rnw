...
development of novel phylogenetic comparative methods. These methods allow
...
a synthesis of phylogenetic knowledge, ultimately leading to a richer understanding of
the history of life, while at the same time providing high-quality phylogenetic information for use
in comparative analyses. However, in practice synthesizing phylogenetic data
is a difficult task. Phylogenetic information is scattered, often only available as image
...
form a comprehensive synthetic tree. Studies can be contributed to the synthetic tree
through a curator interface (\url{https://tree.opentreeoflife.org/curator}),
...
The OTL project provides a web interface to explore, query and
download parts of the synthetic tree (\url{http://tree.opentreeoflife.org})
as well as the published trees that contribute to it. Additionally, the project
provides Application Programming Interfaces (APIs)  to access programmatically
the different services and underlying data.  Here, we present \texttt{rotl}, a
package to interact with OTL's APIs to search for and import phylogenetic trees
and taxonomic information directly in R. Having access to these data in R allows
...
\citealt{Paradis2004}, phytools \citeateu{Revell2012}, geiger
...
comprehensive list). There is also a growing list of packages that allow users
to query and access data from the web directly in R: \hl{rFISHBASE
10.1111/j.1095-8649.2012.03464.x, paleoDB 10.1111/ecog.01154, rAvis
10.1371/journal.pone.0091650 \dots}. By providing direct access to high quality
phylogenetic data in R, \texttt{rotl} fills a key gap in typical comparative
analysis workflows. Moreover, by allowing the gathering, processing, and analysis
of data to be performed programmatically, \texttt{rotl} facilitates reproducible
research practices. 
...
connect to OTL and the public functions users apply to access and process data. 
...
First, we use the function \texttt{tnrs\_match\_names} to match the taxon 
names to their Open Taxonomy identifiers.
<<init, echo=FALSE, purl=FALSE>>=
library(rotl)
@

%First, we use the function \texttt{tnrs\_match\_names} to match the taxon names to their Open Taxonomy identifiers.
...
open data, as well as broaden the adoption of open science practices in
...
species found in GBIF (code available in the Appendix). Another example is available
...
The source for this manuscript is available at \url{https://github.com/fmichonneau/rotl-ms}.
