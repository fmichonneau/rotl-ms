...
\citep{phylobase-0.8.0}, \texttt{phangorn} \citep{schliep2011} and \texttt{RNeXML} \citep{Boettiger2015} each provide
...
\texttt{rotl} gives users access to the endpoints provided by version 2 of the
APIs, and other versions of the APIs can be selected by the user as they become
available (version 3 is scheduled to be released at the end of March 2016). Two
other end-points are available in version 2 of the API: \emph{graph of life} and
\emph{collections}. Data from the \emph{graph of life} endpoint are not intended
for typical users, and will be removed in future releases. The \emph{collection}
endpoint is not fully functional, and no functions are available in the current
release of \texttt{rotl}. Future versions will make them available when the API
implements them.
...
are made available online and accessible using APIs, several packages have been
developed to interact and download these datasets directly in R, thereby
