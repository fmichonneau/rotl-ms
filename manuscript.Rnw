...
analysis. The R packages \texttt{ape} \cite{Paradis2004}, \texttt{phylobase}
\cite{phylobase-0.8.0 }and \texttt{RNeXML} \cite{Boettiger2015} provide
...
\cite{Revell2012}, \texttt{geiger} \cite{Pennell2014-geiger} and \texttt{ggtree} 
\cite{Yu-ggtree} to allow users to analyse and visualise data in a 
...
phylogentic analyses, a growing number of R packages allow users to query and
acess data from the web \hl{(rFISHBASE 10.1111/j.1095-8649.2012.03464.x, 
paleoDB 10.1111/ecog.01154,  rAvis 10.1371/journal.pone.0091650)}, meaning data 
associated with taxa in a given phylogeny can be obtained directly in R. 
These packages, combined the language's support for literature programming 
\hl{(Knuth\_1984, Xie\_2014)} make R an ideal platform for reproducible research
...
phylogenetic and taxonomic data from Open Tree of Life in R. \texttt{rotl} takes
advantage of Application Programming Interfaces (APIs) provided by OTL to acess
sub-trees from the synthetic Open Tree and the published trees that contribute
to the synthesis. By providing direct access to high quality
...
general. For instance, the rOpenSci-developed pacakge \texttt{rTreeBase} allows
users to access phylogenies stored in treebase. \texttt{rotl} contributes to this
...
integration of phylogenetic data retrieved with \texttt{rotl} with other other
data here. We how how we can obtain a map of the occurrences for some of the cat species
(genus \textit{Lynx}) included in the phylogeny above using records for these
species found in GBIF (code available in Appendix)
