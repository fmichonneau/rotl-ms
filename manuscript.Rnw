...
Demonstration (1) getting relationships from a list of taxa

Demonstration (2) getting tree from a given study

* Which studies are available in OTL?
* How to look for them?

Demonstration (3) exploring the taxonomy

Demonstration (4) how does rotl fit into the R package ecosystem?
...
`rotl` is available both from the CRAN repository
