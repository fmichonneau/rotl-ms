...
\citep{Yu-ggtree} allow users to analyze and visualize data in a 
...
phylogenetic and comparative analyses, a growing number of R packages allow users to query and
...
steps taken in gathering, processing, and analyzing a given data set to be
produced.
