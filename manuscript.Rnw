...
Application Programming Interfaces (APIs) to the different services to work
programmatically with the underlying data. Here, we present \texttt{rotl} an R
package to interact with OTL's APIs using R syntax to search for, and import
...
data (e.g., ape, phytools, geiger, ggtree, phylobase, RNeXML, see
\url{https://cran.r-project.org/web/views/Phylogenetics.html} for a
comprehensive list). Furthermore, \texttt{rotl} joins a growing list of R
packages that aim at accessing data from the web.
