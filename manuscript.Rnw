...
structure the comprehensive synthetic tree. Studies can be contributed to the
synthetic tree through a curator interface
...
can be obtained directly in R.  These packages, combined with the language's
support for literate programming (\citealt{Knuth1984}; \citealt{Xie2014}), make
R an ideal platform for reproducible research in phylogenetics and comparative
biology, as they allow a complete record of the steps taken in gathering,
processing, and analyzing a given data set to be produced.
...
\texttt{phylo} objects by \texttt{rotl} using the NEXUS Class Library
...
taxonomic information (i.e. a monotypic taxon).
...
contribute to the synthetic tree. These trees constitute a useful resource to
...
rOpenSci-developed packge \texttt{TreeBase} \citep{Boettiger2012TB} allows users
...
integration of phylogenetic data retrieved using \texttt{rotl} with other data
here. We show how we can obtain a map of the occurrences for some of the cat
species (genus \textit{Lynx}) included in the phylogeny above using records for
these species found in GBIF (Figure~\ref{fig:cat_map}; code available in the
Appendix).
