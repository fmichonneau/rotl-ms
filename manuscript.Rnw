...
and computing technologies enabling the investigation of evolutionary
relationships among taxa. In parallel, new phylogenetic comparative methods have
been developed and make use of these phylogenies to explore fundamental
...
rates, or both \cite{Pennell2013}. The development of these methods has
...
access, synthesize, and integrate into their own research (\cite{Stoltzfus2012},
but see \cite{Cranston2014} for suggestions of best practices).
...
allowing the tree to be updated as relationships are elucidated or
reevaluated. The current draft of the Open Tree of Life contains 2.3 million
tips. Beyond obvious applications across the life sciences to explore research
questions in evolution, biodiversity, and conservation, the resources OTL
provides could be used in education and outreach (e.g., illustrating course
...
present \texttt{rotl} an R package that allows users to interact with OTL's API
to search for, and import phylogenetic trees and taxonomic information into
R. Having access to this data directly in R allows researchers to take advantage
of the multiple R packages that are available to visualize, analyze and
manipulate phylogenetic and comparative data (ape, phytools, geiger, ggtree,
phylobase, RNeXML, etc... cite task view).


\section{Services provided by OTL}

The OTL project provides four resources that serve data to users through the API
(the API endpoints):

\begin{enumerate}
\item The taxonomic information used as the backbone from the tree, the Open
  Tree Taxonomy;
\item The studies and their associated trees some of which are chosen by
  curators to assemble the synthetic tree;
\item A taxonomic name resolution service (TNRS) used to map taxon names to
  the Open Tree Taxonomy;
\item The synthetic tree itself (the Open Tree).
\end{enumerate}
...
OTL's API. Phylogenetic trees served by the API can be imported directly into
R's memory and are represented using the \texttt{ape} \cite{Paradis2004} tree
structure (objects of class \texttt{phylo}) or can be written to files in the
NEXUS, Newick or NeXML file formats. This allows researchers to use these trees
either directly with other R packages, or to be imported in other programs that
can read in phylogenetic tree files.

The synthetic tree currently does not have any branch lengths associated with
it, therefore parametric comparative methods cannot be used on the trees
returned by the API. However, resources and methods are being developed to add
branch lengths information to these topological trees (e.g., \cite{Ksepka2015},
\cite{Pennell2015}). Even without branch lengths, these trees could be useful to
illustrate relationships among species, or to map a trait on a phylogeny.

\texttt{rotl} can also be used to download trees accompanying studies, and that
are used to build the synthetic tree. All the trees from a study are available
even if they do not contribute to the synthetic tree. These trees provide a
useful resource to reproduce or expand on an already published analysis.
...
Use NCL/\texttt{rncl} to parse NEXUS/Newick files.

Taxonomic information, singletons, but NCL can parse it.
