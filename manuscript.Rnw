...
(\cite{Stoltzfus2012}, but see \cite{Cranston2014} for suggestions of best
practices).
...
add branch lengths information to these topological trees (e.g.,
\cite{Ksepka2015}) or use topological trees to identify phylogenetically
equivalent species to increase overlap between chronograms and species trait
data \cite{Pennell2015}. Without branch lengths, these trees remain useful to
illustrate relationships among species, or to map a trait on a phylogeny for
instance.
...
The function \texttt{tnrs\_match\_names} returns a data frame that lists the Open
Tree identifiers as well as other information to help users to ensure that the
...
hemihomonyms. For instance, the argument \texttt{context\_name} can be used to
limit potential matches to a taxonomic group. The function
\texttt{tnrs\_contexts} provides a list of possible taxonomic contexts. When this
strategy cannot be used in case the tree encompasses multiple domains, the
functions \texttt{inspect} and \texttt{update} list alternative matches for a
taxon name, and replace it in the results, respectively. An example of this
approach is provided in the vignette ``How to use rotl?'' that accompanies the
package.
...
been submitted through the curator interface, and also identify the trees that
are being used to assemble the synthetic tree. These trees provide a useful
resource to reproduce or expand on an already published analysis.
...
or trees based on a taxon but other criteria such as the title of the
publication. Here we demonstrate how to look for and retrieve a tree for study
focusing on cats, Felidae (Figure~\ref{fig:plot_cats}).


<<plot_cats, fig.cap="Tree from \\cite{} obtained from OTL">>=
...
cat_tree <- get_study_tree(study_id = cat_studies[["study_ids"]][1],
                           tree_id = list_trees(cat_studies)[[1]][1])
plot(cat_tree)
@
